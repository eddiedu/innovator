sap.ui.define([
	...
	return BaseObject.extend("innovator.books.Books.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias innovator.books.Books.model.GroupSortState
		 */
		...
		/**
		 * Groups by ISBN, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "ISBN") {
				...
				this._oViewModel.setProperty("/sortBy", "ISBN");

				aSorters.push(
					new Sorter("ISBN", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "BookTitle");
			}
